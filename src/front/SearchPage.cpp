    ...
    : QWidget(parent), allItems_(items)
{
    ...
    if (auto* view = qobject_cast<QListView*>(memberFilterCombo_->view()))
    {
        ...
        // Перехватываем клики по popup, чтобы галочки ставились без закрытия списка.
    ...
    filtersLayout->addWidget(memberFilterCombo_, 0, Qt::AlignLeft | Qt::AlignVCenter);
    ...
    // --------- Сигналы ----------
    connect(searchEdit_, &QLineEdit::textChanged, this, &SearchPage::onSearchTextChanged);
...
bool SearchPage::eventFilter(QObject* watched, QEvent* event)
{
    ...
    // Клик по полю "члены предложения" (вся область combo) открывает/закрывает popup.
    if ((watched == memberFilterCombo_ || watched == memberFilterCombo_->lineEdit())
    ...
    if (memberFilterCombo_->view() && watched == memberFilterCombo_->view()->viewport()
        && event->type() == QEvent::MouseButtonRelease)
    {
        ...
        else
        {
            ...
            // Автосинхронизация «Выбрать всё» с фактическим состоянием остальных пунктов.
